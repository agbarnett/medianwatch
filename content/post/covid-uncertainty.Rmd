...
date: 2020-03-17
categories: []
tags: ["statistics","simulation","COVID-19"]
...
R,Recovered
D,Dead')
pander(state.labels, style='simple')
```
...
This work builds heavily on [Alison Hill's model](https://alhill.shinyapps.io/COVID19seir), and I recommend looking at that first.

The table below shows the key model assumptions:
...
The plots below show 50 versions of the cumulative number of infections, recovered and dead by day. Because microsimulations are stochastic, we get a different answer when we re-run the same simulation. So the plots show the stochastic uncertainty in numbers. 
